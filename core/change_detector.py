...
class ChangeDetector:
    """H5 text-first detector with local structure/dimension matching.
    Geometry changes are not reported; structures only define where nearby
    dimensions, GD&T and notes should be compared.
    """
    ...
    @staticmethod
    def _norm(s):
        return re.sub(r"\s+","",s.upper().replace("—","-").replace("–","-").replace("−","-"))
    ...
    def _blocks(self,gray):
        """Create overlapping local search blocks from ink density plus grid.
        Blocks are anchors only and are never emitted as changes.
        """
        h,w=gray.shape[:2]
        _,ink=cv2.threshold(gray,215,255,cv2.THRESH_BINARY_INV)
        # Preserve local drawing clusters without demanding one connected contour.
        ...
        # Overlapping coarse cells guarantee that OCR boxes still have a local
        # anchor even when a CAD drawing consists mostly of thin lines.
        cols=4 if w/h>1.5 else 3
        rows=3 if h/w>1.5 else 2
        sx=w/cols; sy=h/rows
        for r in range(rows):
            for c in range(cols):
                x=max(0,int((c-.15)*sx)); y=max(0,int((r-.15)*sy))
                x2=min(w,int((c+1.15)*sx)); y2=min(h,int((r+1.15)*sy))
        ...
        for b in blocks:
            ...
            for j,m in enumerate(merged):
                if self._iou(b,m)>.25:
                    x=min(b[0],m[0]); y=min(b[1],m[1]); x2=max(b[0]+b[2],m[0]+m[2]); y2=max(b[1]+b[3],m[1]+m[3])
                    merged[j]=(x,y,x2-x,y2-y); hit=True; break
    ...
    def _ocr_pairs(self,before,after,diff,blocks):
        ...
        for bi in range(len(blocks)):
            ...
            for o in gb[bi]:
                ...
                x=max(0,min(ox,nx)-16); y=max(0,min(oy,ny)-16)
                x2=min(diff.shape[1],max(ox+ow,nx+nw)+16); y2=min(diff.shape[0],max(oy+oh,ny+nh)+16)
                local=diff[y:y2,x:x2]
                if local.size and float(np.mean(local>self.pixel_threshold))>=.004:
                    out.append((x,y,x2-x,y2-y,.65+.30*bs))
        return out

    def _raster_fallback(self,before,after,diff,mask,blocks):
        """Find compact changes near text-like ink even when OCR misses the
        changed value. This is deliberately local and conservative."""
        ...
        for i in range(1,n):
            ...
            out.append((x1,y1,x2-x1,y2-y1,.50))
        return out
    ...
    def detect(self,before_page,after_page):
        try:
            ...
            candidates=self._ocr_pairs(before,after,diff,blocks)
            if not candidates:
                candidates=self._raster_fallback(before,after,diff,mask,blocks)
            regions=[]
            ...
            for r in regions:
                ...
                for m in merged:
                    if self._iou((r.x,r.y,r.width,r.height),(m.x,m.y,m.width,m.height))>.15:
                        ...
                        m.old_crop=before[t:bb,l:rr].copy(); m.new_crop=after[t:bb,l:rr].copy(); m.difference_crop=diff[t:bb,l:rr].copy(); m.change_ratio=float(np.mean(m.difference_crop>self.pixel_threshold)); found=True; break
                if not found:merged.append(r)
            return ChangeDetectionResult(True,merged,diff,mask,float(np.mean(mask>0)),"local structure anchor + dimension/GD&T/note matching")
