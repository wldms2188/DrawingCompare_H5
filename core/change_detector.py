...
class ChangeDetector:
    ...
    def _words(self,page):
        try:
            ...
            for z in raw:
                ...
                if text:
                    context=' '.join(lines.get((block_no,line_no),[text])); token_class=self._class(text); context_class=self._class(context)
                    cls=token_class if token_class!='TEXT' else context_class
                    out.append({'text':text,'context':context,'x':x0/r.width,'y':y0/r.height,'w':(x1-x0)/r.width,'h':(y1-y0)/r.height,'class':cls})
    ...
    def _pixel_regions(self,before,after):
        ...
        # Hundreds of tiny components indicate residual alignment/rendering noise, not geometry edits.
    ...
    def detect(self,before_page,after_page,aligned_after=None,alignment_matrix=None):
        try:
            ...
            candidates=[]; maxd=max(80,min(WV,HV)*.08)
            for oi,o in enumerate(oldpx):
                for ni,n in enumerate(mapped):
                    d=float(np.hypot(o['x']+o['w']/2-n['x']-n['w']/2,o['y']+o['h']/2-n['y']-n['h']/2)); bonus=0 if o['class']==n['class'] else 35
            ...
            for o,n,score in pairs:
                ...
                ob=self._word_box(o); nb=self._word_box(n); x=min(ob.x,nb.x);y=min(ob.y,nb.y);xx=max(ob.x+ob.w,nb.x+nb.w);yy=max(ob.y+ob.h,nb.y+nb.h); box=Box(x,y,xx-x,yy-y).pad(5,WV,HV); cls=o['class'] if o['class']!='TEXT' else self._class(o.get('context',o['text'])); kind=self._kind(cls)
                regions.append(ChangeRegion(box.x,box.y,box.w,box.h,box.w*box.h,0.0,kind,max(.5,1-score/max(1,maxd)),self._crop(before,ob),self._crop(view,box),None,o['context'] if cls=='NOTE' else o['text'],n['context'] if cls=='NOTE' else n['text'],kind))
