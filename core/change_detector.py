...
class ChangeDetector:
    ...
    @staticmethod
    def _crop(img, box):
        x,y,xx,yy = box.xyxy(); H,W = img.shape[:2]
        return img[max(0,y):min(H,yy), max(0,x):min(W,xx)]

    @staticmethod
    def _norm_text(text):
        return re.sub(r'\s+', '', str(text).upper().replace('−','-').replace('–','-').replace('—','-'))
    ...
    @staticmethod
    def _find_homography(before, after):
        bg=ChangeDetector._gray(before); ag=ChangeDetector._gray(after)
    ...
    @staticmethod
    def _map_box(q, H, srcW, srcH):
        pts=np.float32([[[q['x']*srcW,q['y']*srcH],[(q['x']+q['w'])*srcW,(q['y']+q['h'])*srcH]]])
        p=cv2.perspectiveTransform(pts,H)[0]
        x,y=float(np.min(p[:,0])),float(np.min(p[:,1])); xx,yy=float(np.max(p[:,0])),float(np.max(p[:,1]))
        return {**q,'x':x,'y':y,'w':max(1,xx-x),'h':max(1,yy-y)}

    @staticmethod
    def _iou(a,b):
        ax,ay,axx,ayy=a.xyxy(); bx,by,bxx,byy=b.xyxy(); x=max(ax,bx);y=max(ay,by);xx=min(axx,bxx);yy=min(ayy,byy)
        inter=max(0,xx-x)*max(0,yy-y); union=a.w*a.h+b.w*b.h-inter
        return inter/max(1,union)

    @staticmethod
    def _word_box(w, pad=8):
        x=int(round(w['x'])); y=int(round(w['y'])); xx=int(round(w['x']+w['w'])); yy=int(round(w['y']+w['h']))
    ...
    def _pixel_regions(self, before, after):
        ...
        a=self._gray(before); b=self._gray(after); diff=cv2.absdiff(a,b)
        _,th=cv2.threshold(diff,self.pixel_threshold,255,cv2.THRESH_BINARY)
        k=np.ones((3,3),np.uint8); th=cv2.morphologyEx(th,cv2.MORPH_OPEN,k); th=cv2.dilate(th,k,1)
        n,_,stats,_=cv2.connectedComponentsWithStats(th,8); out=[]; total=a.shape[0]*a.shape[1]
        for i in range(1,n):
    ...
    def detect(self, before_page, after_page, aligned_after=None):
        try:
            before=self._img(before_page); after_original=self._img(after_page)
            ...
            if H is not None:
                for q in new:
                    try: mapped.append(self._map_box(q,H,after_original.shape[1],after_original.shape[0]))
                    except Exception: pass
            if not mapped: mapped=new
            ...
            for oi,o in enumerate(oldpx):
                for ni,n in enumerate(mapped):
                    ...
                    class_bonus=0 if o['class']==n['class'] else 20
                    if d+class_bonus <= maxd: candidates.append((d+class_bonus,oi,ni))
            ...
            reason=(f'diag: native={len(old)}/{len(new)}, mapped={len(mapped)}, '
                    f'notes={sum(q["class"]=="NOTE" for q in old)}, '
                    f'dimensions={sum(q["class"]=="DIMENSION" for q in old)}, '
                    f'gdt={sum(q["class"]=="GDT" for q in old)}, '
                    f'text_pairs={len(pairs)}, changed_values={sum(r.change_kind!="geometry_change" for r in final)}, '
                    f'pixel_candidates={len(pixels)}, final={len(final)}')
            return ChangeDetectionResult(True,final,None,None,0.0,reason)
        except Exception as exc:
            return ChangeDetectionResult(False,[],reason=f'diag_error: {type(exc).__name__}: {exc}')
