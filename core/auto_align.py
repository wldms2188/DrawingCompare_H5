...
class AutoAlign:
    ...
    def _orb_estimate(self,bg,ag):
        ...
        return M,count,ratio,scale,rotation,'AFFINE'
    def _ecc_estimate(self,bg,ag):
        h,w=bg.shape[:2]; down=min(1.,1400./max(h,w)); size=(max(50,int(w*down)),max(50,int(h*down))); b=cv2.resize(bg,size,interpolation=cv2.INTER_AREA); a=cv2.resize(ag,size,interpolation=cv2.INTER_AREA); b=cv2.GaussianBlur(b,(5,5),0); a=cv2.GaussianBlur(a,(5,5),0); warp=np.eye(2,3,dtype=np.float32); criteria=(cv2.TERM_CRITERIA_EPS|cv2.TERM_CRITERIA_COUNT,300,1e-6)
        try: cc,score=cv2.findTransformECC(b,a,warp,cv2.MOTION_AFFINE,criteria,None,5)
        except cv2.error:return None
        # ECC returns the matrix mapping input(After) coordinates to template(Before) coordinates.
        ...
        if det<=0 or not .75<=s<=1.30 or abs(rot)>self.max_rotation_deg or float(score)<.35:return None
    ...
    def align(self,before_img,after_img):
        ...
        resized=False
        if ag.shape!=bg.shape: after=cv2.resize(after,(w,h),interpolation=cv2.INTER_AREA); ag=self._gray(after); resized=True
        try:
            ...
            if method=='ECC':
                aligned=cv2.warpAffine(after,M,(w,h),flags=cv2.INTER_LINEAR|cv2.WARP_INVERSE_MAP,borderMode=cv2.BORDER_CONSTANT,borderValue=255)
                valid_src=np.full((h,w),255,np.uint8); valid=cv2.warpAffine(valid_src,M,(w,h),flags=cv2.INTER_NEAREST|cv2.WARP_INVERSE_MAP,borderMode=cv2.BORDER_CONSTANT,borderValue=0)
            else:
