...
from config import BEFORE_DIR, AFTER_DIR, OUTPUT_DIR, CONFIG
...
def _normalize_config(config):
    """Keep the GUI compatible with older local config.py copies."""
    page_cfg = getattr(config, "page_match", None)
    if page_cfg is None:
        page_cfg = SimpleNamespace(
            minimum_score=0.55,
            review_score=0.40,
            minimum_feature_matches=8,
        )
        try:
            config.page_match = page_cfg
        except Exception:
            pass
    else:
        if not hasattr(page_cfg, "minimum_score"):
            page_cfg.minimum_score = 0.55
        if not hasattr(page_cfg, "review_score"):
            page_cfg.review_score = 0.40
        if not hasattr(page_cfg, "minimum_feature_matches"):
            page_cfg.minimum_feature_matches = 8
    return config
...
class DrawingCompareApp(tk.Tk):
    def __init__(self):
        ...
        self.before_dir = tk.StringVar(value=BEFORE_DIR)
        self.after_dir = tk.StringVar(value=AFTER_DIR)
        self.output_dir = tk.StringVar(value=OUTPUT_DIR)
        self.status = tk.StringVar(value="Before / After 폴더를 확인한 뒤 비교를 시작하세요.")
        self._build()
    ...
    def _build(self):
        ...
        box = ttk.LabelFrame(root, text="입력 / 출력", padding=12)
        box.pack(fill="x")
        self._path_row(box, "Before", self.before_dir)
        self._path_row(box, "After", self.after_dir)
        self._path_row(box, "결과 폴더", self.output_dir)
    ...
    def _path_row(self, parent, label, variable):
        ...
        ttk.Button(row, text="찾아보기", command=lambda: self.choose(variable)).pack(side="right")
    ...
    def _worker(self, before_dir, after_dir, output_dir):
        try:
            ...
            page_matcher = PageMatcher(runtime_config)
            rows = []
            capture_dir = output_dir / "captures"
            capture_dir.mkdir(parents=True, exist_ok=True)

            for pair_no, (bd, ad) in enumerate(self._match_documents(before_docs, after_docs), 1):
                page_matches = page_matcher.match_pages(bd, ad)
                self.write_log(f"문서 {pair_no}: {bd.filename} ↔ {ad.filename}, 페이지 후보 {len(page_matches)}개")
                for pm in page_matches:
                    ...
                    try:
                        ...
                        if isinstance(alignment, AlignmentResult):
                            ...
                            self.write_log(
                                f"정렬: {alignment.method}, success={alignment.success}, "
                                f"scale={alignment.scale:.3f}, rotation={alignment.rotation:.2f}°, "
                                f"valid={alignment.valid_ratio:.2f}"
                            )
                        else:
                            aligned, matrix = alignment, None
                    except Exception as exc:
                        self.write_log(f"정렬 경고: {exc}")
                        aligned, matrix = ap.image, None

                    result = detector.detect(bp, ap, aligned_after=aligned, alignment_matrix=matrix)
                    self.write_log(f"검출 진단 p{bp.page_index + 1}: {result.reason}")
                    for region_no, region in enumerate(result.regions, 1):
                        ...
                        rows.append({
                            "No": len(rows) + 1,
                            "Before PDF": bd.filename,
                            "After PDF": ad.filename,
                            "Before Page": bp.page_index + 1,
                            "After Page": ap.page_index + 1,
                            ...
                            "Before Text": region.old_text,
                            "After Text": region.new_text,
                            "Before Image": str(old_path),
                            "After Image": str(new_path),
                        })
    ...
    def _match_documents(before_docs, after_docs):
        ...
        for bd in before_docs:
            ...
            best = min(
                remaining,
                key=lambda ad: (
                    0 if Path(bd.filename).stem.lower() == Path(ad.filename).stem.lower() else 10
    ...
    def _write_excel(rows, path):
        ...
        headers = ["No", "Before PDF", "After PDF", "Before Page", "After Page", "Type", "Confidence", "X", "Y", "Width", "Height", "Change Ratio", "Before Text", "After Text", "Before Image", "After Image"]
        for c, h in enumerate(headers, 1):
            ws.cell(1, c, h).font = Font(bold=True)
            ws.cell(1, c).alignment = Alignment(horizontal="center")
        for r, item in enumerate(rows, 2):
            for c, h in enumerate(headers, 1):
                ws.cell(r, c, item.get(h, ""))
            ws.row_dimensions[r].height = 90
            for col, key in ((15, "Before Image"), (16, "After Image")):
                p = item.get(key)
                if p and Path(p).exists():
                    img = XLImage(p)
                    img.width, img.height = 150, 80
                    ws.add_image(img, f"{chr(64 + col)}{r}")
        wb.save(path)

    def _finished(self, report, count):
        self.progress.stop()
        self.run_btn.configure(state="normal")
        self.status.set(f"완료 — 변경영역 {count}개")
