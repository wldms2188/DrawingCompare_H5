...
class DrawingCompareApp(tk.Tk):
    ...
    def start(self):
        ...
        if not before.is_dir() or not after.is_dir():
            messagebox.showwarning("입력 확인", "Before와 After 폴더를 모두 선택하세요."); return
        self.run_btn.configure(state="disabled"); self.progress.start(12); self.status.set("PDF를 읽고 있습니다...")
    ...
    def _worker(self, before_dir, after_dir, output_dir):
        try:
            ...
            rows, capture_dir = [], output_dir / "captures"
            ...
            for pair_no, (bd, ad) in enumerate(pairs, 1):
                ...
                for bp, ap in page_pairs:
                    ...
                    result = detector.detect(bp.image, aligned)
                    for region_no, region in enumerate(result.regions, 1):
                        stem = f"D{pair_no:02d}_P{bp.page_index + 1:03d}_R{region_no:03d}"
                        old_path, new_path = capture_dir / f"{stem}_before.png", capture_dir / f"{stem}_after.png"
                        cv2.imwrite(str(old_path), self._to_bgr(region.old_crop)); cv2.imwrite(str(new_path), self._to_bgr(region.new_crop))
            ...
            self.write_log(f"ERROR: {exc}"); self.after(0, lambda e=str(exc): self._failed(e))
    ...
    def _match_documents(before_docs, after_docs):
        ...
        for bd in before_docs:
            ...
            best=min(remaining,key=lambda ad:(0 if Path(bd.filename).stem.lower()==Path(ad.filename).stem.lower() else 10)+abs(bd.page_count-ad.page_count))
            pairs.append((bd,best)); remaining.remove(best)
        return pairs
    ...
    def _match_pages(before_pages, after_pages):
        ...
        for bp in before_pages:
            if not remaining: break
            def score(ap):
                area1=max(bp.width*bp.height,1); area2=max(ap.width*ap.height,1)
                return abs(bp.aspect_ratio-ap.aspect_ratio)+abs(area1-area2)/area1
    ...
    def _write_excel(rows,path):
        ...
        wb=Workbook(); ws=wb.active; ws.title="Drawing Compare"
    ...
    def _finished(self,report,count):
        self.progress.stop(); self.run_btn.configure(state="normal"); self.status.set(f"완료 — 변경영역 {count}개")
